...
export function BentoDashboard({ resources }: BentoDashboardProps) {
	...
	return (
		<div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-12 gap-4 lg:gap-6 auto-rows-[minmax(120px,auto)]">
			...
			{/* HERO WIDGET: Cluster Overview (col-span-8, row-span-3) */}
			<div className="col-span-1 md:col-span-4 lg:col-span-8 lg:row-span-3 bg-card border border-border rounded-2xl shadow-sm overflow-hidden flex flex-col relative group">
				...
				<div className="p-6 border-b border-border/50 flex justify-between items-center z-10">
					...
					<div className="flex items-center gap-4">
						<div className="flex flex-col items-end">
							<span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">99.9%</span>
					...
					<ResponsiveContainer width="100%" height="100%">
						<LineChart data={chartData}>
							...
							<Line 
								...
								strokeWidth={3}
								dot={false}
								activeDot={{ r: 6, fill: '#C7654F', stroke: 'var(--background)', strokeWidth: 2 }}
							/>
							<Line 
								type="monotone" 
								dataKey="memory" 
								...
								strokeWidth={3}
								dot={false}
								activeDot={{ r: 6, fill: '#d97706', stroke: 'var(--background)', strokeWidth: 2 }}
							/>
						</LineChart>
					</ResponsiveContainer>
				</div>
			...
			<div className="col-span-1 md:col-span-4 lg:col-span-4 lg:row-span-4 bg-card border border-border rounded-2xl shadow-sm flex flex-col overflow-hidden">
				...
				<div className="flex-1 overflow-y-auto p-5 space-y-4">
					{/* Mock feed items */}
					{[...Array(5)].map((_, i) => (
						<div key={i} className="flex gap-3 relative before:absolute before:left-2.5 before:top-6 before:bottom-[-16px] before:w-px before:bg-border last:before:hidden">
							...
							<div className="pb-2">
								<p className="text-sm font-medium text-foreground">
									<code className="text-xs bg-muted px-1 py-0.5 rounded font-mono text-primary mr-1 border border-border">api-gateway</code>
			...
			<div className="col-span-1 md:col-span-4 lg:col-span-8 lg:row-span-2 bg-card border border-border rounded-2xl shadow-sm flex flex-col overflow-hidden">
				<div className="p-5 border-b border-border/50 flex justify-between items-center">
					...
					<span className="text-xs font-mono bg-muted px-2 py-1 rounded-md text-muted-foreground border border-border">{activeResources} Active</span>
				</div>
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-border/50 p-px">
					{resources.slice(0, 8).map((resource) => (
						<div key={resource.id.toString()} className="bg-card hover:bg-muted/30 p-4 transition-colors cursor-pointer flex flex-col justify-between min-h-[100px]">
							<div className="flex items-start justify-between">
								<h4 className="font-semibold text-sm truncate pr-2">{resource.name}</h4>
			...
			<div className="col-span-1 md:col-span-4 lg:col-span-4 lg:row-span-1 bg-card border border-border rounded-2xl shadow-sm p-5 flex flex-col justify-center relative overflow-hidden">
